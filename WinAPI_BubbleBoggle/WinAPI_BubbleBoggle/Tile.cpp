...
namespace MomDra
{
	Tile::Tile(const Vector2& pos, std::shared_ptr<Texture> texture, const Layer& layer) noexcept : Object{ pos, Vector2{TILE_SIZE_X, TILE_SIZE_Y}, layer }, texture{ texture }, imgIndex{ 0 }
	{
	...
	void Tile::Render(const HDC& hdc) const noexcept
	{
		...
		int maxCol{ width / TILE_SIZE_X };
		int maxRow{ height / TILE_SIZE_Y };
